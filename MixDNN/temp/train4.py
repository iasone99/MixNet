...
def batch_mels_wav(data, mel_spectrogram, num_frames, tts):
    ...
    wavs_target=wavs_clean
    return mel, mel_target, wavs_target
...
def main():
    ...
    # loss
    loss_function = mixLoss.MixLoss4()
    # optimizer
    ...
    for epoch in range(hp.epochs):
        ...
        for i, data in enumerate(data_loader):
            ...
            mel, mel_target, wavs_target = batch_mels_wav(data=data, mel_spectrogram=mel_spectrogram, num_frames=num_frames,
                                             tts=tts)
            ...
            # compute the loss
            loss = loss_function(mel_pred, wavs_target, mel_target)
            ...
            if global_step % hp.save_step == 0:
                t.save({'model': model.state_dict(),
                       ...
                       os.path.join(args.checkpoint_dir, 'checkpoint_MixNetLoss3_%d.pth.tar' % save_step))
...
def test(model, test_loader, mel_spectrogram, num_frames, loss_function, tts, chunk_size):
    """
    ...
    for i, data in enumerate(test_loader):
        ...
        # only consider the non-padded interval of the spec
        mask[mask != mask] = 0
        mel = torch.permute(mel, (0, 2, 1))  # [B,F,T]
        mel_target = torch.permute(mel_target, (0, 2, 1))  # [B,F,T]
