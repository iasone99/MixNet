...
from MixDNN import mixCNN, sliceDNN
...
def main():
    ...
    model = sliceDNN.SliceDNN(hidden_size=hp.hidden_size_DNN, num_layers=hp.layers_DNN,
                              input_len=2 * hp.num_mels,
                              output_len=hp.num_mels).to(hp.device)
    print("This model has " + str(sum(p.numel() for p in model.parameters() if p.requires_grad)) + " parameters")
    # loss
    loss_function = mixLoss.MixLossSlice()
    # optimizer
    ...
    for epoch in range(hp.epochs):
        ...
        for i, data in enumerate(data_loader):
            ...
            # compute the loss
            loss = loss_function(mel_pred, mel_target, mel_noise, mel_tts)
            ...
            if global_step % hp.save_step == 0:
                t.save({'model': model.state_dict(),
                       ...
                       os.path.join(args.checkpoint_dir, 'checkpoint_SliceNN_%d.pth.tar' % save_step))
            ...
            plt.savefig("loss_gradientsSlice.svg")
...
def test(model, test_loader, mel_spectrogram, num_frames, loss_function, tts):
    ...
    for i, data in enumerate(test_loader):
        ...
        # compute the loss
        test_loss = loss_function(mel_pred, mel_target, mel_noise, mel_tts)
